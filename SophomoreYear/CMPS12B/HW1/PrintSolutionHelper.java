...
class PrintSolutionHelper {
	...
	// A cool method that actually prints a chessboard with queens onto the console.
	// Uses some UNICODE awesomeness
	// Input: 2D boolean array isFilled, where isFilled[i][j] is true iff there is a
	// queen at col i, row j.
	...
	private static void printSolution(boolean[][] isFilled) {
		String line = "  "; // starting strings to be printed. line is the dividing horizontal strip along
							// the board
		...
		for (int i = board_size; i > 0; i--) // loop over all the rows in decreasing order. each iteration will print a
												// row
		{
			...
			for (int j = 1; j < board_size + 1; j++) // now loop over columns to create individual squares
			{
				...
				if (isFilled[i][j]) // if (i,j) has a queen
					pieces = pieces + "|" + background + "\u2655 " + "\u001B[0m"; // put UNICODE symbol for queen with a
																					// line. also set background, and
																					// then apply ANSI reset code
				else
					pieces = pieces + "|" + background + "  " + "\u001B[0m"; // put a space and set background, reset as
																				// before
			}
	...
	// Output: true, if line represented sequence of queens, and false if anything
	// else detected
	...
	public static void main(String[] args) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) { // reading in line, and checking if file has ended
				...
				if (readFromSolutionFile(line)) // this parses line. it returns true and populates the data
												// structures/fields of class, if line was sequence of queen positions
